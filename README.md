...
def fix_bittrex_dates(df_crypto):
    ...
    df_crypto['Date'] = pd.to_datetime(df_crypto['Date'], format="%Y-%m-%d %I-%p", errors='ignore')
...
def crypto_plot(thisdf):
    ...
    plt.savefig(''.join(('output/',str(pd.to_datetime(thisdf.Date.values[0])).replace(':','-'), ' to ',str(pd.to_datetime(thisdf.Date.values[-1])), ' Bittrex_NEOETH_1h.png')).replace(':','-'))
    plt.show()
```
